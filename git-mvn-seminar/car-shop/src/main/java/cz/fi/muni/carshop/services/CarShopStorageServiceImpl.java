...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
		List<Car> carsOfType = CarShopStorage.getInstancce().getCars().get(car.getType());
		if (carsOfType == null || !carsOfType.contains(car)){
			throw new RequestedCarNotFoundException("Car not found in the database");
		} else {
	...
	public void addCarToStorage(Car car) {
		if (car.getPrice() < 0){
			throw new IllegalArgumentException("Car's price can not be negative!");
		}
